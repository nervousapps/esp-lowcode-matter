...
namespace LD2410
{
  ...
  const uint8_t headData[4]{0xF4, 0xF3, 0xF2, 0xF1};
...
{
  rx_read_len = simple_uart_read_bytes(UART_PORT_NUM, q.arr, LD2410_BUFFER_SIZE, 10);
  q.front = 0;
  q.rear = rx_read_len;
...
bool MyLD2410::sendCommand(const uint8_t *command)
{
  ...
  simple_uart_write_bytes(UART_PORT_NUM, LD2410::headConfig, 4, 500);
  simple_uart_write_bytes(UART_PORT_NUM, &command, size, 500);
  simple_uart_write_bytes(UART_PORT_NUM, LD2410::tailConfig, 4, 500);
  printf("\n##### Command sent\n");
