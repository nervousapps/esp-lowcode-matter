...
void check_contact_state(lp_sw_timer_handle_t timer, void *args) {
    int new_contact_state = system_digital_read(SENSOR_PIN);
    if(contact_state != new_contact_state) {
        contact_state = new_contact_state;
        set_contact_state(contact_state);
    }
    ESP_LOGD("", "Contact after : %d", contact_state);
}
...
static void setup()
{
    ...
    lp_sw_timer_config_t timer_check_contact_state_cfg = {
        .periodic = true,
        .timeout_ms = 500,
        .handler = check_contact_state,
        .arg = &contact_state,
    };
    lp_sw_timer_handle_t timer_check_contact_state = lp_sw_timer_create(&timer_check_contact_state_cfg);

    lp_sw_timer_start(timer_check_contact_state);
}
